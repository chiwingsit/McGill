...
public class IndexedHeap{   
	...
	private void swap(int i, int j){
		...
		names.set(j, tmpName);
		priorities.set(j, tmpPriority);
		nameToIndex.put(tmpName, j);
	...
	public String removeMin(){
		...
		names.set(1, names.get(sizePQ()));
		names.remove(sizePQ());
		
		priorities.set(1, priorities.get(sizePQ()));
		priorities.remove(sizePQ());
		
		nameToIndex.put(names.get(sizePQ()), 1);
		nameToIndex.remove(minName);	
	...
	public void  add(String name, double priority){
		...
		//----------------------- ADD YOUR CODE HERE  ----------------------------		names.add(name);
		names.add(name);
		priorities.add(priority);
		nameToIndex.put(name, sizePQ());
