...
// was just to test that things were working. Feel free to delete it or just
// leave
// it there.

class Exptree extends Bintree {
	public Exptree(Object root) {
		super(root);
	};
...
// The different kinds of nodes that appear in the tree are different classes
// that
// extend Exptree. The oper node has a character in its root and two subtrees
// while
// the var node just has a character in its root. Variable names are single
// characters!
// The point is that the type can be used in the parser to switch between the
// cases.
// Make sure you understand how instanceof works.

class Oper extends Exptree {
	public Oper(Character o, Exptree l, Exptree r) {
		super(o, l, r);
	};
}

class Var extends Exptree {
	public Var(Character q) {
		super(q);
	};
}
...
class Parser {
	...
	public Exptree parse() {
		...
		while(sym == '+'){
			getsym();
			result = new Oper(new Character('+'), temp, parse());
		}
		return result;
	}
	...
	public Exptree term() {
		...
		while(sym == '*'){
			getsym();
			result = new Oper(new Character('*'), temp, term());
	...
	public Exptree primary() {
		...
		if(Character.isLetter(sym)){
			result = new Var(new Character(sym));
			getsym();
			return result;
		}
		else if(sym == '('){
	...
	// file, then constructs the parscursore tree and prints preorder and inorder
...
//		')';
//		Data[5] = '*'; Data[6] = '('; Data[7] = 'C'; Data[8] = '+'; Data[9] =
//		'D';
//		Data[10] = ')'; Data[11] = '$';
...
class Codegen {
	...
	private void codegen(char operator, Exptree t) {
		...
		if (t instanceof Var) {
			switch ((int) operator) {
			case ((int) '='):
				System.out.println("LOAD " + t.root());
				break;
			case ((int) '+'):
				System.out.println("ADD " + t.root());
				break;
			case ((int) '*'):
				System.out.println("MUL " + t.root());
				break;
			}
		} else {
			...
			if(t.right() instanceof Oper){
				...
				System.out.println("STORE " + tempstore);
				codegen('=', (Exptree) (t.right()));
				if(t.root().equals(new Character('+')))
					System.out.println("ADD " + tempstore);
				if(t.root().equals(new Character('*')))
					System.out.println("MUL " + tempstore);
				tempstore--;
			}
			else{
